...
guard_direction = np.rec.fromrecords([(direction, move) for direction, move in {'N': (-1, 0), 'E': (0, 1), 'S': (1, 0), 'W': (0, -1)}.items()], dtype=[('name', 'U1'), ('move', tuple)])

def location_on_map(guard_location):
    return np.all([index in range(size) for index, size in zip(guard_location, lab_map.shape)])
